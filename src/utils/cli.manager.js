...
class CliAuthManager {
    ...
    async initiateDeviceFlow() {
        ...
        try {
            const response = await fetch(`${config.apibaseurl}/api/v1/oauth2/device/code`, {
                method: 'POST',
                headers: {
    ...
    async authorizeLogin(user_code, access_token) {
        try {
            const response = await fetch(`${config.apibaseurl}/api/v2/oauth2/authorize`, {
                method: 'POST',
                headers: {
    ...
    async pollForToken(device_code, code_verifier) {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            try {
                const response = await fetch(`${config.apibaseurl}/api/v1/oauth2/token`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                        Accept: 'application/json',
