...
const handleImageVideoCompletion = async (req, res) => {
    ...
    try {
        ...
        const response_data = await axios.post(`${config.apibaseurl}/api/v2/chat/completions?chat_id=${chat_id}`, reqBody, {
            headers: {
...
const getVideoTaskStatus = async (videoTaskID, token) => {
    try {
        const response_data = await axios.get(`${config.apibaseurl}/api/v1/tasks/status/${videoTaskID}`, {
            headers: {
                "Authorization": `Bearer ${token}`,
                'Content-Type': 'application/json',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
